...
class QA_Model():
  def load_data(self, debug=False):
    ...
    encoded_data = [{'context': [[self.vocab.encode(word.lower()) 
                                  for word in sent]
    ...
    self.config.num_context = max([len(par['context']) for par in dataset])
    self.config.num_questions = max([len(par['qas']) for par in dataset])

    encoded_data = [{'context': np.array([[0 for _ in range(self.config.num_steps - len(sent))] \
                                          + sent
                                          for sent in par['context']] \
    ...
    n = floor(self.config.training_rate * len(encoded_data))
    m = floor((1-self.config.training_rate) * len(encoded_data) / 2)
    self.encoded_train = encoded_data[:n]
    self.encoded_valid = encoded_data[n:n+m]
    self.encoded_test = encoded_data[n+m:]
...
class RNNContext_Model(QA_Model):
  ...
  def run_epoch(self, session, data, train_op=None, verbose=10):
    ...
    if verbose:
      sys.stdout.write('\r')
    return np.exp(np.mean(total_loss)), pred, answers
...
def test_RNNLM():
  ...
  with tf.Session() as session:
    ...
    for epoch in range(config.max_epochs):
      ...
      start = time.time()
      train_pp, train_pred, train_true = model.run_epoch(
          session, model.encoded_train,
          train_op=model.train_step)
      valid_pp, valid_pred, valid_true = model.run_epoch(session, model.encoded_valid)
      train_pred = np.argmax(train_pred, 1)
      valid_pred = np.argmax(valid_pred, 1)
      print('Training perplexity: {}'.format(train_pp))
      print('Training accuracy: {}'.format(np.sum(train_pred==train_true)/len(train_true)))
      print('Validation perplexity: {}'.format(valid_pp))
      print('Validation accuracy: {}'.format(np.sum(valid_pred==valid_true)/len(valid_true)))
    ...
    test_pp, test_pred, test_true = model.run_epoch(session, model.encoded_test)
    test_pred = np.argmax(test_pred, 1)
    print('=-=' * 5)
    print('Test perplexity: {}'.format(test_pp))
    print('Test accuracy: {}'.format(np.sum(test_pred==test_true)/len(test_true)))
    print('=-=' * 5)
